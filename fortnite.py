"""
Commons Clause” License Condition v1.0 Copyright gummyloo909 2020
...
with open('config1.json') as f:
    print(f' [BOT] [{getTime()}] Loading config.')
    data = json.load(f)
    print(f' [RenegadeFN] [{getTime()}] Config loaded.')
    
debug = 'False'
if debug == 'True':
    print(f' [RenegadeFN] [{getTime()}] Debug logging is on.')
    debugOn()
else:
    print(f' [RenegadeFN] [{getTime()}] Debug logging is off.')
...
async def event_ready():
    print(Fore.GREEN + ' [RenegadeFN] [' + getTime() + '] Client ready as {0.user.display_name}.'.format(client))
...
async def event_party_invite(invite):
    if data['joinoninvite'].lower() == 'true':
        if invite.sender.display_name not in data['BlockList']:
            try:
                await invite.accept()
                print(Fore.GREEN + f' [RenegadeFN] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
            except Exception as e:
                pass
        elif invite.sender.display_name in data['BlockList']:
            print(Fore.GREEN + f' [RenegadeFN] [{getTime()}] Never accepted party invite from' + Fore.RED + f' {invite.sender.display_name}')
    if data['joinoninvite'].lower() == 'false':
        if invite.sender.display_name in data['FullAccess']:
            await invite.accept()
            print(Fore.GREEN + f' [RenegadeFN] [{getTime()}] Accepted party invite from {invite.sender.display_name}')
        else:
            print(Fore.GREEN + f' [RenegadeFN] [{getTime()}] Never accepted party invite from {invite.sender.display_name}')
...
async def event_friend_request(request):
    if data['friendaccept'].lower() == 'true':
        if request.display_name not in data['BlockList']:
            try:
                ...
                print(f" [RenegadeFN] [{getTime()}] Accepted friend request from: {request.display_name}")
            except Exception as e:
                pass
        elif request.display_name in data['BlockList']:
            print(f" [RenegadeFN] [{getTime()}] Never Accepted friend reqest from: " + Fore.RED + f"{request.display_name}")
    if data['friendaccept'].lower() == 'false':
        if request.display_name in data['FullAccess']:
            ...
            print(f" [RenegadeFN] [{getTime()}] Never accepted friend request from: {request.display_name}")

@client.event
async def event_party_member_join(member):
    if client.user.display_name != member.display_name:
        print(f" [RenegadeFN] [{getTime()}] {member.display_name} has joined the lobby.")
        ...
        if message.author.display_name in data['FullAccess']:
            ...
            if len(args) >= 3:
                ...
                try:
                    ...
                    pass
                    print(f" [RenegadeFN] [{getTime()}] Can't find user: " + color.GREEN + f"{joinedArgumentsAdmin}" + color.END)
...
try:
    ...
    print(Fore.RED + f" [RenegadeFN] [{getTime()}] [ERROR] {e}")
